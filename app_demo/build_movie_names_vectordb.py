...
with open("/content/drive/MyDrive/movie_titles.txt", "r", encoding="utf-8") as f:
    movies = [line.strip() for line in f]

# Load embeddings 
import numpy as np
embeddings = np.load("/content/drive/MyDrive/movie_title_embeddings.npy")

#  Populate IDs list (list of indices for movies)
ids = [str(i) for i in range(len(movies))]

# Set up client
client = chromadb.PersistentClient("/content/drive/MyDrive/chroma_db") # Whatever file path your current chromadb file is 
